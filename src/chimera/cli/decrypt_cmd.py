"""chimera decrypt — RC4 / XOR a captured or embedded blob with a known key.

The reusable last step of a network-capture challenge once the key is recovered.
Backed by chimera.symcrypt.
"""
...
@click.option("--algo", type=click.Choice(["rc4", "xor"]), default="rc4", show_default=True)
...
def decrypt(data: str | None, in_file: str | None, key: str, algo: str,
            in_encoding: str, key_encoding: str, as_json: bool):
    """Decrypt DATA (or --in-file) with --key using RC4 or XOR."""
