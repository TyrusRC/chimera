"""HTML presentation layer for the chimera report payload.

`chimera.report.builder.build_report` produces a JSON-serialisable dict;
the helpers in this module turn that dict into a single self-contained
HTML page an analyst can open offline. The split keeps presentation
(CSS, table layout, escape rules) out of the data layer so a future
SPA-rendered view can consume the same payload without dragging the
HTML string-builder along for the ride.
"""
